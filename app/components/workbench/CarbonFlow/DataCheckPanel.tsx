  ...
  const columns: TableProps<Node<NodeData>>['columns'] = [
    ...
    },
    {
      ...
      children: [
        ...
        { title: '运输-起点', dataIndex: ['data', 'startPoint'], key: 'startPoint', width: 120, align: 'center', render: (text: any, record: Node<NodeData>) => record.type === 'distribution' ? ((record.data as DistributionNodeData).startPoint || '-') : '-' },
        { title: '运输-终点', dataIndex: ['data', 'endPoint'], key: 'endPoint', width: 120, align: 'center', render: (text: any, record: Node<NodeData>) => record.type === 'distribution' ? ((record.data as DistributionNodeData).endPoint || '-') : '-' },
        { title: '运输方式', dataIndex: ['data', 'transportationMode'], key: 'transportationMode', width: 100, align: 'center', render: (text: any, record: Node<NodeData>) => record.type === 'distribution' ? ((record.data as DistributionNodeData).transportationMode || '-') : '-' },
        { 
          title: '数据来源', 
    ...
    },
    {
      title: '背景数据 (排放因子)',
      children: [
        { title: '名称', dataIndex: ['data', 'carbonFactorName'], key: 'carbonFactorName', width: 150, align: 'center', render: (v: any) => v || '-' },
        { title: '数值 (kgCO2e)', dataIndex: ['data', 'carbonFactor'], key: 'carbonFactor', width: 130, align: 'center', render: (v: any) => v || '-' },
        { title: '单位', dataIndex: ['data', 'carbonFactorUnit'], key: 'carbonFactorUnit', width: 100, align: 'center', render: (v: any) => v || '-' },
        { title: '地理代表性', dataIndex: ['data', 'emissionFactorGeographicalRepresentativeness'], key: 'emissionFactorGeographicalRepresentativeness', width: 110, align: 'center', render: (v: any) => v || '-' },
        { title: '时间代表性', dataIndex: ['data', 'emissionFactorTemporalRepresentativeness'], key: 'emissionFactorTemporalRepresentativeness', width: 100, align: 'center', render: (v: any) => v || '-' },
        { title: '数据库名称', dataIndex: ['data', 'carbonFactordataSource'], key: 'carbonFactordataSource', width: 120, align: 'center', render: (v: any) => v || '-' },
        { title: '因子UUID', dataIndex: ['data', 'factorUUID'], key: 'factorUUID', width: 180, align: 'center', render: (v: any) => v || '-' },
         { 
          title: '质量评分', 
      ...
      children: [
        { 
            ...
            render: (status: string) => {
                let color = 'default';
                if (status === '已验证' || status === '内部验证') color = 'success';
                if (status === '未验证') color = 'error';
                if (status === '待验证') color = 'processing';
